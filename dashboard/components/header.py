...
def render_cockpit_header(status: dict | None, history_df: pd.DataFrame) -> None:
    """Renderiza a barra superior de status operacional do cockpit."""
    ...
    # Avaliação do status global da operação
    if total_runs == 0:
        ...
        system_status_label = f"Circuit Breaker: {quarantined_count} incidentes isolados"
        status_icon = icon_shield_alert(14, "currentColor")
    else:
        system_status_cls = "chip-emerald"
        system_status_label = "100% SAUDÁVEL"
        status_icon = icon_shield_check(14, "currentColor")

    # Avaliação de frescor (Freshness / SLA)
    if not history_df.empty and "recorded_at" in history_df.columns:
        ...
        if "T" in latest_time:
            ...
            freshness_text = f"Última execução: {date_part} {time_part} UTC"
        else:
            freshness_text = f"Última execução: #{history_df.iloc[0].get('run_id', '')[:10]}"
    else:
    ...
    api_icon = icon_activity(14, "currentColor")

    db_cls = "chip-blue" if db_ok else "chip-rose"
    db_text = f"BANCO: {db_type.upper()}" if db_ok else "BANCO: não conectado"
    db_icon = icon_database(14, "currentColor")

    ai_cls = "chip-cyan" if gemini_ok else "chip-blue"
    ai_text = f"Modelo: Gemini Flash 3.8" if gemini_ok else "Modelo: Regras Locais"
    ai_icon = icon_cpu(14, "currentColor")

    badge_b64 = _get_badge_base64()
    badge_html = (
        f'<img src="{badge_b64}" class="brand-badge-img" alt="DataOps AI Badge" />'
        if badge_b64
        else icon_shield_check(28, "#0F172A")
    )
    ...
    st.markdown(
        ...
        <div class="cockpit-header-bar">
            <div class="cockpit-brand">
                {badge_html}
                <div>
                    <span class="editorial-bar"></span>
                    <span class="cockpit-tag">Central de observação</span>
                    <div class="cockpit-title" role="heading" aria-level="1">Monitoramento de incidentes</div>
                </div>
            </div>
            <div class="cockpit-badges">
                ...
                <span class="chip {db_cls}">{db_icon} {db_text}</span>
                <span class="chip {ai_cls}">{ai_icon} {ai_text}</span>
